...
_cu_left, _cu_right = st.columns([2,3])
with _cu_left:
...
with _cu_right:
    st.caption("Bitte Kundendaten links eingeben. Diese erscheinen im PDF‑Export.")
...
# =============================
# PDF‑Export (Diagramme, Parameter, Auswertung)
# =============================
def _fig_cf_png(_df):
    fig, ax = plt.subplots(figsize=(6,3))
    d=_df[_df["Jahr"]>0].copy()
...
def _fig_cum_png(_df):
    fig, ax = plt.subplots(figsize=(6,3))
    d=_df[_df["Jahr"]>0].copy(); d["Kumuliert [€]"]=d.groupby("Szenario")["Netto Cashflow"].cumsum()
    for scen, s in d.groupby("Szenario"):
        ax.plot(s["Jahr"], s["Kumuliert [€]"], label=scen)
...
def _build_pdf():
    ...
    try:
        ...
        _line(f"EEG [ct/kWh]: {eeg_feed_ct}; MS‑Zuschlag [ct/kWh]: {mieterstrom_premium_ct}")
        _line(f"EV‑Anteil Basis [%]: {sc_share_base}; Override [%]: {grid_share_override}")
        _line(f"Inflation/Preiswachstum [%/a]: {global_infl} / {price_growth}")
        _line(f"Diskontsatz [%/a]: {discount}")
        _line(f"NPV GGV [€] (LG): {_eur(npv_ggv)}")
        _line(f"NPV MS [€] (LG): {_eur(npv_ms)}")
        _line(f"Payback GGV / MS [a]: {('n/a' if pb_ggv is None else pb_ggv)} / {('n/a' if pb_ms is None else pb_ms)}")
        if irr_ggv is not None and irr_ms is not None:
            _line(f"IRR GGV / MS [%]: {irr_ggv*100:.2f} / {irr_ms*100:.2f}")
    except Exception as e:
        _line(f"(Hinweis: Einige Parameter/KPIs konnten nicht gelesen werden: {e})")
    c.showPage()

    # Charts
    _draw_header(c, "Charts")
    try:
        img1 = ImageReader(_fig_cf_png(df_all))
        img2 = ImageReader(_fig_cum_png(df_all))
...
with _pdf_col1:
    if st.button("📄 PDF‑Report erstellen", type="primary"):
        ...
        st.download_button("Download PDF", data=_pdf_bytes, file_name="Projektbericht_GGV_vs_MS.pdf", mime="application/pdf")
with _pdf_col2:
    st.caption("Der PDF‑Report enthält: Kundendaten (Cover), Parameter & KPIs sowie Charts.")
