# app.py (v7 – Korrekturen Cap/EEG/Premium, Vollversorgung, IRR)
import streamlit as st
import pandas as pd
import numpy as np
import plotly.express as px

st.set_page_config(page_title="GGV vs. Mieterstrom – Szenariorechner (v7)", layout="wide")
...
def irr_from_df(df):
    cf = df.sort_values("Jahr")["Netto Cashflow"].values.astype(float)
    lo, hi = -0.99, 2.0
    def npv(r):
...
with st.sidebar.expander("Mieterstrom – Vollversorgung & Index-Toggles", expanded=False):
    ...
    index_eeg = st.checkbox("EEG-Einspeisevergütung jährlich indexieren", value=False)
...
with c1:
    st.markdown(f"<div style='border:1px solid #e5e7eb; padding:10px; border-radius:12px; text-align:center;'><div style='font-weight:600;'>Mieterstrom‑Preisdeckel [ct/kWh]</div><div style='font-size:22px;'>{cap_now:.1f}</div></div>", unsafe_allow_html=True)
with c2:
    st.markdown(f"<div style='border:1px solid #e5e7eb; padding:10px; border-radius:12px; text-align:center;'><div style='font-weight:600;'>NPV GGV [€]</div><div style='font-size:22px;'>{eur2(ne_npv_ggv)}</div></div>", unsafe_allow_html=True)
with c3:
    st.markdown(f"<div style='border:1px solid #e5e7eb; padding:10px; border-radius:12px; text-align:center;'><div style='font-weight:600;'>NPV Mieterstrom [€]</div><div style='font-size:22px;'>{eur2(ne_npv_ms)}</div></div>", unsafe_allow_html=True)
with c4:
    st.markdown(f"<div style='border:1px solid #e5e7eb; padding:10px; border-radius:12px; text-align:center;'><div style='font-weight:600;'>Payback: GGV / MS</div><div style='font-size:22px;'>{pb1} / {pb2}</div></div>", unsafe_allow_html=True)

st.markdown("<div style='height:6px;'></div>", unsafe_allow_html=True)
st.markdown("**LG – Liegenschaft (kumuliert)**")
d1, d2, d3, d4 = st.columns(4)
with d1:
    st.markdown(f"<div style='border:1px solid #c7d2fe; padding:10px; border-radius:12px; text-align:center; background:#eef2ff;'><div style='font-weight:700; color:#1f4acc;'>Mieterstrom‑Preisdeckel [ct/kWh]</div><div style='font-size:22px; font-weight:700; color:#1f4acc;'>{cap_now:.1f}</div></div>", unsafe_allow_html=True)
with d2:
    st.markdown(f"<div style='border:1px solid #c7d2fe; padding:10px; border-radius:12px; text-align:center; background:#eef2ff;'><div style='font-weight:700; color:#1f4acc;'>NPV GGV [€]</div><div style='font-size:22px; font-weight:700; color:#1f4acc;'>{eur2(npv_ggv)}</div></div>", unsafe_allow_html=True)
with d3:
    st.markdown(f"<div style='border:1px solid #c7d2fe; padding:10px; border-radius:12px; text-align:center; background:#eef2ff;'><div style='font-weight:700; color:#1f4acc;'>NPV Mieterstrom [€]</div><div style='font-size:22px; font-weight:700; color:#1f4acc;'>{eur2(npv_ms)}</div></div>", unsafe_allow_html=True)
with d4:
    st.markdown(f"<div style='border:1px solid #c7d2fe; padding:10px; border-radius:12px; text-align:center; background:#eef2ff;'><div style='font-weight:700; color:#1f4acc;'>Payback: GGV / MS</div><div style='font-size:22px; font-weight:700; color:#1f4acc;'>{pb1} / {pb2}</div></div>", unsafe_allow_html=True)
...
r1.metric(f"ROI (Cash-on-Capex) GGV bis Jahr {years}", f"{(roi_ggv*100):.1f}%")
r2.metric(f"ROI (Cash-on-Capex) MS bis Jahr {years}", f"{(roi_ms*100):.1f}%")
r3.metric("IRR GGV", f"{(irr_ggv*100):.2f}%" if irr_ggv is not None else "n/a")
r4.metric("IRR Mieterstrom", f"{(irr_ms*100):.2f}%" if irr_ms is not None else "n/a")
...
fig_roi = px.line(roi_curve.melt(id_vars="Jahr", var_name="Szenario", value_name="ROI [%]"),
                  x="Jahr", y="ROI [%]", color="Szenario", title="ROI-Verlauf (Eigentümer) – Cash-on-Capex")
st.plotly_chart(fig_roi, use_container_width=True)

st.caption("Hinweis: **ROI = kumulierter Netto‑Cashflow / CAPEX** (keine Jahres‑Rendite); **IRR** = interne Verzinsung. EEG & Mieterstrom‑Zuschlag sind standardmäßig **nicht** indexiert. MS‑Preis wird **jährlich** gegen den Cap geprüft. OPEX startet in Jahr 1. Optional: Vollversorgung inkl. Restbezug einkaufen.")
